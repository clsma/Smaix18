...
'<br />' + tinyMCEPopup.getLang('jbimages_dlg.maybe_an_error', 0) + '<br /><a href="#" onClick="jbImagesDialog.showIframe()">' 
+ tinyMCEPopup.getLang('jbimages_dlg.view_output', 0) + '</a>';
	...
	//alert(result2.resultCode);
		...
		else
		{
			...
			/*try{
			tinymce.editor.execCommand('mceInsertContent', false, '<img src="' + result.filename +'" />');
			tinymce.close();
		    
			}catch(asdasd){
				var editor = tiny.getInstanceById(tinyMCE.id); 
				if(editor){ 
                    editor.execCommand('mceInsertContentAt', false , '<img src="' + result.filename +'" />'); // value is selected from the popup
				}
			}*/
			alert(error);
			}
