...
calOffsetX = (screen.width-200)/2
calOffsetY = (screen.height-200)/2
...
function calOBJ(name, title, field) {
  this.name = name;
  this.title = title;
  this.field = field;
}
...
function addCalendar(name, title, field) {
  cals[cals.length] = new calOBJ(name, title, field);
}
...
function changeCal(d) {
  ...
  if (currentCal != null) {
    ...
    if (document.getElementById(currentCal.field).value!="" && calRE.test(document.getElementById(currentCal.field).value)) {
      var cd = getDateNumbers(document.getElementById(currentCal.field).value);
...
function showCal(name) {
  ...
  if (currentCal != null && document.getElementById(currentCal.field)) {
    var calRE = getFormat();
    ...
    if (calWin != null && !calWin.closed) {
      hasCal=true;
      calWin.moveTo(calOffsetX,calOffsetY);
    }

    if (!hasCal) {
      if (isIE || isOpera6) {
        calWin=window.open("","cal","toolbar=0,width="+calWidth+",height="+calHeight+",left="+(calOffsetX)+",top="+(calOffsetY));
      }
      else {
        calWin=window.open("","cal","toolbar=0,width="+calWidth+",height="+calHeight+",screenx="+(calOffsetX)+",screeny="+(calOffsetY));
      }
    }
...
function clearDate() {
  document.getElementById(currentCal.field).value = "";
  hideCal();
}
...
function pickDate(d) {
  ...
  document.getElementById(currentCal.field).value = date;
...
function checkDate(name) {
  ...
  if (thisCal != null && document.getElementById(thisCal.field)) {
    var calRE = getFormat();

    if (calRE.test(document.getElementById(thisCal.field).value)) {
      return 0;
    }
    else {
      return 1;
