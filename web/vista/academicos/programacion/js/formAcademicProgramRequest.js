...
clsma.$requestSetup({
    controller: [
        {name: Rutac + '/Adm/AcademicProgramRequest', path: 'event'}
    ],
    returning: 'object'
});
...
function getGroups(type) {
    ...
    clsma.type = type;
    clsma.$request({
        data: ['GROUPS', {a: clsma.dks, b: clsma.type, c: clsma.pkp, d: clsma.idepgm, agnprs:clsma.agnprs, prdprs:clsma.prdprs}],
        loading: true
    }).done(function (data) {
...
function setGen(a, b, c) {
    if (c.STDPKP==='Proyectar-') {
        ...
        img.attr({
            title: 'Proyectar'
            , style: 'width:24px;height:24px'
            , onclick: 'genCoursesProject(\'' + c.IDESMT + '\',\'' + c.IDEPGM + '\',\'' + c.SMTPSM+'\')'
            , src: Rutav + '/vista/img/Actualizar.png'
        }).appendTo(div);
        return div.html();
...
function genProject(idepgm,b) {
   ...
   clsma.tab.enableTab(2, 3, 4).activeTab(2);
   lstSmt();
}
...
// Editar programacion academica Proyectada
function setEditProject(a, b, c) {
    if (c.NROPKP!=='NEW' && c.STDPKP === 'Abierta') {        
        var state = 'PROJ';
        var img = $('<img/>');
        var div = $('<div/>');
        img.attr({
            title: 'Modificar'
            , style: 'width:24px;height:24px'
            , onclick: 'editProject(\''+c.IDEPGM+'\')'
            , src: Rutav + '/vista/img/botones/menuCollapse.png'        
        }).appendTo(div);
        return div.html();
    } else {
        return '';
...
function genCoursesProject(ideSmt , idePgm, smtpsm ) {
  ...
  clsma.$confirm('¿Desea proyectar [ %s ] curso(s) en el semestre [ %s ] - Reforma [ %s]?'
            ...
            .Aceptar(function () {
                //'PROJECT'
        clsma.$request({
            data: ['PROJ-SMT', {id: ideSmt, pgm: idePgm, qrs: nrocrs, psm: nropsd, sem: smtpsm}],
            loading: true
        }).then(function (data) {
            //var resp = $.parseJSON(data);
            if(data.exito==='OK'){                
