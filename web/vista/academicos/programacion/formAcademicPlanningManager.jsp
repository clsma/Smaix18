...
<%@page import="mvc.controller.AdmAcademicPlanningManagerAction"%>
<%@include file="/vista/formPageIncludes.jsp" %>
<%    String usable = Util.getStrRequest("usable", request);
    usable = AdmAcademicPlanningManagerAction.validRol(model, usable);
    String cal = AdmAcademicPlanningManagerAction.validCalendar(model);
%>
<html>
    <head>
        <script src="<%=request.getRequestURI().substring(0, request.getRequestURI().lastIndexOf("/") + 1)%>js/formAcademicPlanningManager.js?v=ssss" charset="UTF-8"></script>
        ...
        <script>
            ...
            clsma.agn = '<%=AdmAcademicPlanningManagerAction.agnprs%>';
            clsma.prd = '<%=AdmAcademicPlanningManagerAction.prdprs%>';
        </script>
    </head>
    <body>
